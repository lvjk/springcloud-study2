...
public class BuildDao {
	...
	public List<Build> selectAll() {
		try {			
			return jdbcTemplate.query("select * from build", new BuildRowMapper());
	 ...
	 */
	@Transactional(readOnly = true)
	public Build selectOne(Serializable id) {
		try {			
			return jdbcTemplate.queryForObject("select * from build where id = ?", new Object[] { id }, Build.class);
		} catch (DataAccessException e) {
			throw e;
		}
...
class BuildRowMapper implements RowMapper<Build> {
	@Override
	public Build mapRow(ResultSet rs, int rowNum) throws SQLException {
		Build bean = new Build();		
