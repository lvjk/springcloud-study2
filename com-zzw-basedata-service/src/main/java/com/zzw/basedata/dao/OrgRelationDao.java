...
import com.zzw.domain.basedata.OrgRelation;

@Repository(value = "orgRelationDao")
public class OrgRelationDao {
	@Autowired
	...
	public List<OrgRelation> selectAll() {
		try {			
			return jdbcTemplate.query("select * from org_relation", new OrgRelationRowMapper());
	 ...
	 */
	@Transactional(readOnly = true)
	public OrgRelation selectOne(Serializable id) {
		try {			
			return jdbcTemplate.queryForObject("select * from org_relation where id = ?", new Object[] { id }, OrgRelation.class);
		} catch (DataAccessException e) {
			throw e;
		}
...
class OrgRelationRowMapper implements RowMapper<OrgRelation> {
	@Override
	public OrgRelation mapRow(ResultSet rs, int rowNum) throws SQLException {
		OrgRelation bean = new OrgRelation();		
