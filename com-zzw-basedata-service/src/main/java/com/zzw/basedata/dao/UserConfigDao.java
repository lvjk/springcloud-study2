...
@Repository(value = "userConfigDao")
public class UserConfigDao {
	...
	public List<UserConfig> selectAll() {
		try {			
			return jdbcTemplate.query("select * from user_config", new UserConfigRowMapper());
	 ...
	 */
	@Transactional(readOnly = true)
	public UserConfig selectOne(Serializable id) {
		try {			
			return jdbcTemplate.queryForObject("select * from user_config where id = ?", new Object[] { id }, UserConfig.class);
		} catch (DataAccessException e) {
			throw e;
		}
...
class UserConfigRowMapper implements RowMapper<UserConfig> {
	@Override
	public UserConfig mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		bean.setOpendoorunalert(rs.getBoolean("opendoorunalert"));
		bean.setRemark(rs.getString("remark"));
		bean.setUncalltime(rs.getString("uncalltime"));
		bean.setUserId(rs.getString("user_id"));
		bean.setWxset(rs.getString("wxset"));
		return bean;
	}
