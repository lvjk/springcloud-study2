...
import com.zzw.domain.basedata.Cell;
...
public class CellDao {
	...
	public List<Cell> selectAll() {
		try {			
			return jdbcTemplate.query("select * from cell", new CellRowMapper());
	 ...
	 */
	@Transactional(readOnly = true)
	public Cell selectOne(Serializable id) {
		try {			
			return jdbcTemplate.queryForObject("select * from cell where id = ?", new Object[] { id }, Cell.class);
		} catch (DataAccessException e) {
			throw e;
		}
...
class CellRowMapper implements RowMapper<Cell> {
	@Override
	public Cell mapRow(ResultSet rs, int rowNum) throws SQLException {
		Cell bean = new Cell();		
		bean.setId(rs.getString("id"));
		bean.setAddress(rs.getString("address"));
		bean.setCallphone(rs.getString("callphone"));
		bean.setCancall(rs.getBoolean("cancall"));
